...
public class Logic
    ...
    public double calculate(int size, int count) {
        ...
        for(int i=0; i<count;i++){
            ...
            HashSet<Integer>datesOfBirthdays= new HashSet<>();
            Random object= new Random();
            object.setSeed(i);


            for(int j=0; j<size;j++){
                int birthdayDate= object.nextInt(365);
                if(datesOfBirthdays.contains(birthdayDate)){
                    sameBirthday++;
                    break;
                }
                else{
                    datesOfBirthdays.add(birthdayDate);
                }
            }
        }
        double percentage= (sameBirthday/count) *100;
        return percentage;
